...
public class IRhino3dmExporter extends IRhino3dm{
    ...
    public static int version = 4; // Rhino version 4 format
    public final static int openNurbsVersion4 = 201004095;
    public final static int openNurbsVersion5 = 202105077;

    public static int openNurbsVersion = openNurbsVersion4;
    ...
    // setting static int version. if using (Rhino3dmFile)file, set file.version
    public static void setVersion(int ver){
	// either version 4 or version 5
	if(ver==4){
	    version = 4;
	    openNurbsVersion = openNurbsVersion4;
	}
	else if(ver==5 || ver==50){
	    version = 50;
	    openNurbsVersion = openNurbsVersion5;
	}
	else{
	    IOut.err("IRhino3dmExporter can write version 4 or 5 currently");
	}
	IOut.debug(10, "version = "+version);
    }


    // using static int version. if possible use file.sizeofChunkLength().
    public static int SizeofChunkLength(){ return version<50? 4:8; }
    ...
    */
    public static boolean write(File file, IServerI server, int version3dm){
	try{
	    //FileOutputStream fos = new FileOutputStream(file);
	    IRandomAccessOutputStream fos = new IRandomAccessOutputStream(file);
	    if(fos==null) return false;
	    boolean retval = write(fos, server,version3dm);
	    fos.close();
	    return retval;
    ...
    public static boolean write(OutputStream os, IServerI server, int version3dm) throws IOException{
	IRhino3dmExporter exporter = new IRhino3dmExporter(os,server);
	exporter.write(version3dm);
	return true;
    }

    /*
    public void write() throws IOException{
	IOut.debug(10, "write : version="+version); //
	write(version); // static int version
    }
    */

    public void write(int vers) throws IOException{
	setVersion(vers);
	// vers != version
	...
	String versionNumText = String.valueOf(file.version);
	...
	IOut.debug(10, "writing " + objectNum + " objects");
	...
	if(raostream!=null){
	    ...
	    currentPos += objectTable.getByteLength(); // written byte length
	}
	else{
	...
	else if(e instanceof IVolume){
	    obj = getRhinoBrep( (IVolume)e );
	}
    ...
    static public Brep getRhinoBrep(IVolume volume){ return new Brep(volume); }
...
//	writeChunkTable(tcodeUserTable); // hide to debug 20210617
	...
	if(file.sizeOfChunkLength()==4){
	    int len = currentPos + 4 + 4 + 4; /*header+body+int content*/
	    writeInt32(cos, len, cos.getCRC());
	}
	else{
	    int len = currentPos + 4 + 8 + 8; /*header+body+int content*/
	...
	*/
	...
	//if(s==null){ IOut.err("string is null"); return; }
	...
	*/
    ...
    public void writeChunkValue(int typecode, int value) throws IOException{
	if(file.sizeOfChunkLength()==8) writeInt64((long)value, null);
	else writeInt32(value, null);
    }

    public static void writeChunkValue(OutputStream os, int typecode, int value) throws IOException{
	if(SizeofChunkLength()==8) writeInt64(os, (long)value, null);
	else writeInt32(os, value, null);
    }

    public void writeChunkLength(int length)throws IOException{
	if(file.sizeOfChunkLength()==8) writeInt64((long)length, null);
	else writeInt32(length, null);
    }

    public static void writeChunkLength(OutputStream os, int length)throws IOException{
	if(SizeofChunkLength()==8) writeInt64(os, (long)length, null);
	else writeInt32(os, length, null);
    }
	...
	if(!chunk.isShort()&&chunk.doCRC()){
	    //writeInt32(chunk.body+4,null);
	    writeChunkValue(chunk.header, chunk.body+4);
	}
	else{
	    //writeInt32(chunk.body,null);
	    writeChunkValue(chunk.header, chunk.body);
	}
	...
	if(!chunk.isShort()&&chunk.doCRC()){
	    //writeInt32(os,chunk.body+4,null);
	    writeChunkValue(os, chunk.header, chunk.body+4);
	}
	else{
	    //writeInt32(os,chunk.body,null);
	    writeChunkValue(os, chunk.header, chunk.body);
	}
    ...
    */
	...
	public Chunk getChunk(){
	    ...
	    return new Chunk(header, content.length, content, crc);
	}
    }
